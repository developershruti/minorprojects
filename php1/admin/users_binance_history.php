...
if(is_post_back()) {
	...
	if(is_array($arr_pay_ids)) {
		...
		if(isset($_REQUEST['Payment_Paid']) || isset($_REQUEST['Payment_Paid_x']) ){
			$sql_update="update ngo_binance_deposit set pay_status='Paid',pay_transaction_no='$pay_transaction_no',pay_transfer_date=ADDDATE(now(),INTERVAL 630 MINUTE),pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
		}else if(isset($_REQUEST['Payment_Unpaid']) || isset($_REQUEST['Payment_Unpaid_x']) ){
			$sql_update="update ngo_binance_deposit set pay_status='Unpaid',pay_transaction_no='',pay_transfer_date='',pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
...
//$columns = "select u_username,u_fname,u_account_phrase,u_city,u_id,u_ref_userid,u_bank_register,ngo_binance_deposit.*  ";
//$sql = " from ngo_users,ngo_binance_deposit where  u_id=pay_userid and u_status='Active'";
$columns = "select  *  ";
$sql = " from ngo_binance_deposit where 1 ";
if ($pay_plan!='') 		{$sql .= " and pay_plan='$pay_plan' ";} 
if ($pay_status!='') 	{$sql .= " and pay_status='$pay_status' ";}
if ($pay_coin!='') 		{$sql .= " and pay_group='$pay_group' ";}
...
if ($pay_address!='') 	{$sql .= " and pay_address like '%$pay_address%' ";}
if ($pay_network!='') 	{$sql .= " and pay_network like '%$pay_network%' ";}
if ($pay_status!='') 	{$sql .= " and pay_status='$pay_status' ";}
if ($pay_txId!='') 		{$sql .= " and pay_txId like '%$pay_txId%' ";}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td id="pageHead"><div id="txtPageHead">8G Pay Nodal History </div></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td id="content"><form method="get" name="form2" id="form2" onsubmit="return confirm_submit(this)">
        <table width="750"  border="0" align="center" cellpadding="2" cellspacing="0" class="tableSearch">
          ...
          <!--<tr>
			...
			//echo array_dropdown( $ARR_CURRENCY_GROUP, $pay_plan,'pay_plan', 'class="txtbox"  style="width:120px;"','--select--');
          ...
          <tr>
            ...
            <td width="128"><input name="pay_txId"style="width:120px;" type="text" value="<?=$pay_txId?>" />            </td>
          ...
          <tr>
            ...
            <td><input name="pay_address"style="width:120px;" type="text" value="<?=$pay_address?>" /></td>
            <td align="right" valign="middle"><!--<input name="export" type="checkbox" id="export" value="1" />--></td>
            <td valign="middle"><!--Export list--> </td>
          </tr>
          <tr>
...
while ($line_raw = mysqli_fetch_array($result)) {
 ...
 ?>
          <tr class="<?=$css?>">
            ...
            <td nowrap="nowrap"><?=$pay_coin?></td>
            <td nowrap="nowrap"><?=$pay_network?> </td>
            <td nowrap="nowrap"><?=$pay_address?> </td> 
			 <td nowrap="nowrap"><?=price_format($pay_amount)?></td>
            <td nowrap="nowrap"><?=date_format2($pay_date)?></td>
            <td nowrap="nowrap"><?=$pay_txId?></td>
           
            <td align="center"><a title="<?=$pay_txId?>" href="#"> <?=$pay_status?> </a></td>
			 <td nowrap="nowrap"><?=db_scalar("select CONCAT(u_username) from ngo_users where u_id = '$pay_userid'");?></td>
