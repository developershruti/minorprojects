...
#print_r($_POST);
	...
	if(is_array($arr_pay_ids)) {
		...
		if(isset($_REQUEST['Payment_Paid']) || isset($_REQUEST['Payment_Paid_x']) ){
				...
				$sql_test = "select  *  from  ngo_users_reward_point, ngo_users  where pay_userid=u_id and pay_id in ($str_pay_ids) ";
  				$result_test = db_query($sql_test);
 				while ($line_test= mysqli_fetch_array($result_test)){
					@extract($line_test);
					$payout_count1 = db_scalar("select count(pay_id) from ngo_users_payment where  pay_userid = '$u_id' and  pay_refid ='$pay_id' and pay_plan='REGENERATION_POOL_INCOME' ")+0;
					if($payout_count1==0){
					$pay_for1 = "Re-Generation Income From Reference ID : $pay_id "; 
					if($pay_amount==15) { $pay_amount2=50;} else { $pay_amount2=$pay_amount*3; }
					$sql_earning = "insert into ngo_users_payment set pay_drcr='Cr',pay_userid ='$u_id' ,pay_refid ='$pay_id', pay_group='REGENERATION_POOL_INCOME' , pay_plan='REGENERATION_POOL_INCOME' ,pay_for = '$pay_for1' ,pay_ref_amt='$pay_amount' ,pay_unit = 'F' ,pay_rate = '$pay_amount', pay_amount = '$pay_amount2' ,pay_date=ADDDATE(now(),INTERVAL 330 MINUTE) ,pay_datetime =ADDDATE(now(),INTERVAL 330 MINUTE) ";
					db_query($sql_earning);
					}
			  ...
			  $sql_update="update ngo_users_reward_point set pay_status='Close',pay_transaction_no='$paid_transaction_no',pay_transfer_date='$paid_date',pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
			...
			$sql_update="update ngo_users_reward_point set pay_status='Unpaid',pay_transaction_no='',pay_transfer_date='',pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
		
		}else if(isset($_REQUEST['Payout_SMS']) || isset($_REQUEST['Payout_SMS_x']) ) {
			// sms code start
			 	#$sql_test = "select * from  ngo_users_reward_point, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
				/*$sql_test = "select u_username,u_fname,u_mobile, sum(pay_amount)as payment  from  ngo_users_reward_point, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
  			 ...
  			 // sms code start
				...
				$sql_test = "select u_username,u_fname,u_mobile,  pay_amount  as payment  from  ngo_users_reward_point, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids) ";
...
$columns = "select u_username,u_fname,u_city,u_id,u_ref_userid,u_bank_register,ngo_users_reward_point.*  ";
$sql = " from ngo_users,ngo_users_reward_point where  u_id=pay_userid ";
//and u_status='Active'
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td id="pageHead"><div id="txtPageHead"> Users Re-Generation List </div></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td id="content"><form method="get" name="form2" id="form2" onsubmit="return confirm_submit(this)">
        <table width="750"  border="0" align="center" cellpadding="2" cellspacing="0" class="tableSearch">
          ...
          <tr>
            ...
            <td width="96" align="right">Package : </td>
            <td width="161">
              <input name="pay_group"style="width:50px;" type="text" value="<?=$pay_group?>" /></td>
          </tr>
          <tr>
            ...
            <td width="128" value=""><!--<input name="pay_status"style="width:120px;" type="text" value="<?=$pay_status?>" />--></td>
      ...
      <form method="post" name="form1" id="form1" onsubmit="confirm_submit(this)">
        <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="tableList">
          <tr>
            ...
            <th width="4%" >City</th>
            <th width="4%" >RG PayID </th>
            <th width="15%" >Naration</th>
            <th width="4%" >RG Package</th>
            <th width="4%" >Payment Type </th>
            <th width="4%" >Ref Amt </th>
            <th width="7%" >RG Amount </th>
            <th width="6%" >RG Date </th>
...
?>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="34%" align="right" style="padding:2px"><h2>
            </td>
            <td width="34%" align="right" style="padding:2px"> Total Fund ($):
              <?=$pay_amount_total;?>
              </h2>
            </td>
            ...
            <td width="22%" align="right" style="padding:2px"> Paid date:
              <?=get_date_picker("paid_date", date("Y-m-d"))?></td>
            <td width="34%" align="right" style="padding:2px"> Transaction No/:
              <input name="paid_transaction_no" type="text" size="50"  />
            </td>
