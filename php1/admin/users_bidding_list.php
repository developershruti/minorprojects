...
if ($bid_status!='') 		{$sql .= " and bid_status='$bid_status' ";}
            ...
            <form method="get" name="form2" id="form2" onsubmit="return confirm_submit(this)">
              <table width="676"  border="0" align="center" cellpadding="2" cellspacing="0" class="tableSearch">
                ...
                <tr>
                  ...
                  <td width="137" align="right">Game No: </td>
                  <td width="244">  <input name="bid_gameno"style="width:120px;" type="text" value="<?=$bid_gameno?>" /> 
				  <?
                ...
                <tr>
                  <td align="right" class="tdLabel"><span class="td_box">Bidding  On<span class="error">*</span></span></td>
                  <td><span class="td_box">
                    <?=array_dropdown($ARR_BID_GROUP, $bid_plan, 'bid_plan');?>
                  </span></td>
              ...
              <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="tableList">
                <tr>
					<th width="10%" >Bid ID</th>
						<th width="10%" >&nbsp;Bid Gameno</th>
					<th width="10%" >Bidder User ID <?= sort_arrows('u_username')?></th>
					<th width="10%" >Bidder Name <?= sort_arrows('u_fname')?></th>
					<th width="10%" >&nbsp;Capital Balance</th>
 					<th width="10%" >&nbsp;Plan No</th>
 					<th width="10%" >&nbsp;Trade Amount<?= sort_arrows('bid_amount')?></th>
					<th width="10%" >&nbsp;Trade Date <?= sort_arrows('bid_date')?></th>
					<th width="10%" >&nbsp;Draw Date </th>
					<th width="10%" >&nbsp;Trade Status</th>
...
while ($line_raw = mysqli_fetch_array($result)) {
	...
	$capital_balance =  db_scalar("SELECT  (SUM(IF(pay_drcr='Cr',pay_amount,''))-SUM(IF(pay_drcr='Dr',pay_amount,''))) as Balance FROM ngo_users_ewallet where pay_userid='$u_id' and pay_group='CW'")+0;			
			
	 $css = ($css=='trOdd')?'trEven':'trOdd';
  ?>
		<tr class="<?=$css?>">
		<td nowrap="nowrap"><?=$bid_id?></td>
		<td nowrap="nowrap"><?=$bid_gameno?></td>
 		<td nowrap="nowrap"><?=$u_username?></td>
		<td nowrap="nowrap"><?=$u_fname?> </td>
		<td nowrap="nowrap"><?=$capital_balance?> </td>
 		<td nowrap="nowrap"><?//=$bid_plan?> <img width="50" src="../images/number/<?=$ARR_BID_IMG[$line['bid_plan']]?>"></td>
  		<td nowrap="nowrap"><?=$bid_amount?></td>
 		<td nowrap="nowrap"><?=($bid_datetime)?></td>
		<td nowrap="nowrap"><?=($bid_draw_date)?></td>
