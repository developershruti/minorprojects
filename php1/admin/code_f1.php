 ...
 if ($code_cunter<=500) {
	if ($code_username!='' && $code_cate!='') {
		...
		#$code_string = md5(uniqid(rand(), 1));
 		$code_string = 'AD'.strtoupper(substr(md5(uniqid(rand(), 1)),0,10));


		//code_usefrom =now() ,
 		  $sql = "insert into ngo_code set  code_userid='$code_userid',code_username='$code_username', code_creqid='$code_creqid' ,code_cate = '$code_cate', code_string = '$code_string', code_plan='$code_plan',code_state='$code_state', code_date = ADDDATE(now(),INTERVAL 750 MINUTE) ,code_useto=ADDDATE(now(),INTERVAL 30 DAY) ,code_user='$_SESSION[sess_admin_login_id]', code_group='$code_group' ";
		db_query($sql);
		$arr_code_ids[] = mysqli_insert_id();
		} 
			$str_code_ids = implode(',', $arr_code_ids);
			$log_count = $code_cunter;
			$sql2 = "insert into ngo_code_log set  log_userid= '0' ,log_sent_userid= '$code_userid' ,log_sent_username='$code_username',log_count='$log_count' ,log_string='$str_code_ids', log_status='Active' ,log_date=ADDDATE(now(),INTERVAL 750 MINUTE), log_user='admin', log_group='PIN' ";
			db_query($sql2);
...
<form name="form1" method="post" enctype="application/x-www-form-urlencoded" <?= validate_form()?>>
  <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0" class="tableForm">
	 ...
	 <!-- <tr>
	  ...
	  <tr>
	    <td class="tdLabel">Assign To User ID </td>
