...
protect_admin_page2();
print_r($_SESSION);
if($pay_action=='IMPS' && $pay_id!='' && $u_username!='' && $pay_status=='Unpaid' && $imps_amount>=1){

  if($_SESSION['bizz_token']=='') {
    header("Location: bizz91_setting_update.php");
    exit;
  }

  print  $response =  bizz91_send_payout($u_username,$imps_amount,$pay_id , $_SESSION['bizz_token'] , $_SESSION['bizz_status']);
   $u_username='';
...
//
			...
			$sql_update="update ngo_users_payment set pay_status='Unpaid' ,pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
		} else if(isset($_REQUEST['Payout_Delete']) || isset($_REQUEST['Payout_Delete_x']) ){
		 			 
				$sql_test = "select *  from  ngo_users_payment where pay_plan='BANK_WITHDRAW' and pay_status='Unpaid' and pay_id in ($str_pay_ids)  ";
 	            ...
 	            db_query("delete  FROM ngo_users_payment where pay_userid='$line_test[pay_userid]' and pay_plan='BANK_WITHDRAW'  and pay_id='$line_test[pay_id]' ");
			...
			db_query($sql_update);  
				...
				/*$sql_test = "select *  from  ngo_users_payment where pay_plan='BANK_WITHDRAW' and pay_id in ($str_pay_ids)  ";
  				$result_test = db_query($sql_test);
 				while ($line_test= mysqli_fetch_array($result_test)){
...
if ($pay_status=='')  {$sql .= " and pay_status='Unpaid'";} 
...
$sql_export_total = $sql ." group by u_bank_acno "; 
...
while ($line_raw = mysqli_fetch_array($result)) {
	...
	/*if ($pay_plan=='BANK_WITHDRAW') {
		$withdrawal_paid = db_scalar("select count(*) from ngo_users_payment where pay_userid =$u_id and pay_plan='BANK_WITHDRAW' and pay_status='Paid'")+0;
		$withdrawal_unpaid = db_scalar("select count(*) from ngo_users_payment where pay_userid =$u_id and pay_plan='BANK_WITHDRAW' and pay_status='Unpaid'")+0;
 ...
 ?>
	...
	<? if ($show=='Yes') { ?> <td><?=$pay_id_refno?></td> <? } ?>
      ...
      <td nowrap="nowrap"><?=$pay_amount*70?></td>
      ...
      <td nowrap="nowrap"><a href="users_with.php?pay_action=IMPS&pay_id=<?=$pay_id?>&imps_amount=<?=round($pay_amount*70,0)?>&u_username=<?=$u_username?>&pay_status=Unpaid">Paynow </td>
