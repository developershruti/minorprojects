...
#print_r($_POST);
	...
	if(is_array($arr_pay_ids)) {
		...
		if(isset($_REQUEST['Payment_Paid']) || isset($_REQUEST['Payment_Paid_x']) ){
			$sql_update="update ngo_users_coin_ewallet set pay_status='Paid',pay_transaction_no='$pay_transaction_no',pay_transfer_date=ADDDATE('$pay_transfer_date',INTERVAL 1 MINUTE),pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
 		
		}else if(isset($_REQUEST['Payment_Unpaid']) || isset($_REQUEST['Payment_Unpaid_x']) ){
			$sql_update="update ngo_users_coin_ewallet set pay_status='Unpaid',pay_transaction_no='',pay_transfer_date='',pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
		
		}else if(isset($_REQUEST['Payout_SMS']) || isset($_REQUEST['Payout_SMS_x']) ) {
			// sms code start
			 	#$sql_test = "select * from  ngo_users_coin_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
				/*$sql_test = "select u_username,u_fname,u_mobile, sum(pay_amount)as payment  from  ngo_users_coin_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
  			 ...
  			 // sms code start
				...
				$sql_test = "select u_username,u_fname,u_mobile,  pay_amount  as payment  from  ngo_users_coin_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids) ";
...
$columns = "select u_username,u_fname,u_city,u_id,u_ref_userid,u_bank_register,ngo_users_coin_ewallet.*  ";
$sql = " from ngo_users,ngo_users_coin_ewallet where  u_id=pay_userid ";
//and u_status='Active'
...
<? include("top.inc.php");?>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td id="pageHead"><div id="txtPageHead"> Users E-Wallet details </div></td>
        </tr>
      </table>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td id="content"> 
			...
			<!--<div align="right"> <a href="users_ewallet_drcr_f.php">Transfer Fund in e-Wallet</a></div> -->
...
?>
             ...
             <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                <td width="34%" align="right" style="padding:2px"><h2> Total Fund ($): <?=$pay_amount_total;?> </h2>
