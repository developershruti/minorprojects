...
if(is_post_back()) {
	...
	if(is_array($arr_code_ids)) {
		...
		if(isset($_REQUEST['Delete']) || isset($_REQUEST['Delete_x']) ) {
			...
			# $message ="Code alocated to Username".$u_id." Name:".$name." Code Sl No:".$str_code_ids."";
...
if ($code_username!='') {
 ...
 if ($log_id!='') {
	$log_string = db_scalar("select log_string from ngo_code_log where log_id='$log_id'");
	if ($log_string!='') {
		$sql .= " and code_id in ($log_string) ";
	}	
 }
...
if ($export3=='1') {
 	$arr_columns =array('u_username'=>'Username','u_fname'=>'User Name','code_is'=>'Pin Status','code_cate'=>'Pin Type','COUNT( * ) AS Total_Used_Pin'=>'Total Issue Pin');	
...
if ($export4=='1') {
 	$arr_columns =array('u_username'=>'Username','u_fname'=>'User Name','code_usefrom'=>'Issue Date','code_is'=>'Pin Status','code_cate'=>'Pin Type','COUNT( * ) AS Total_Used_Pin'=>'Total Issue Pin');	
  ...
  <tr>
    <td id="content">
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td width="870"><table width="93%"  border="0" align="center" cellpadding="1" cellspacing="1" class="tableSearch">
              <tr align="center">
                <th colspan="8">Search</th>
              </tr>
			     <tr>
                <td width="118" align="right" class="tdLabel">Receiver User ID:</td>
                <td width="126"><input name="code_username" class="txtLight" type="text" value="<?=$code_username?>" /></td>
                <td width="86"   align="right">Transfered By   ID </td>
                <td width="120"><input name="code_transfer_userid" class="txtLight" type="text" value="<?=$code_transfer_userid?>" /></td>
                <td width="65" align="right"  >Code Is :  </td>
                <td width="120"  ><select name="code_is">
                  <option value="" selected="selected">All</option>
                  <option value="Available">Available</option>
                  <option value="Used">Used</option>
                </select></td>
			     <td width="20"  ><input name="export2" type="checkbox" id="export2" value="1" /></td>
			     <td width="155"  > Issue Pin Summary </td>
			     </tr>
			  <tr>
                <td align="right" class="tdLabel">Auto User ID From/To : </td>
                <td nowrap="nowrap"><input name="code_id_from" type="text" class="txtLight" value="<?=$code_id_from?>" size="10" />
              ...
              <tr>
                ...
                <td><?
		 $sql ="select utype_id , utype_name from ngo_users_type where utype_value>=10 order by utype_id asc";  
		...
		?></td>
			...
			?>
                ...
                <td align="right">&nbsp;</td>
                <td>&nbsp;</td>
                <td><input name="export" type="checkbox" id="export" value="1" /></td>
                <td> Download Pin List </td>
              </tr>
            </table></td>
            <td width="285" valign="top">
			
			<table width="94%" border="1" align="center" cellpadding="2" cellspacing="2" style="border:dotted; border-color:#CCCCCC; background-color:#cdcdcd;">
              <tr>
                <td>&nbsp;</td>
                <td><strong>Registration</strong></td>
                 
                <td><strong>SMS Pro.</strong> </td>
              </tr>
              <tr>
                <td width="44%"><strong>Pin From Admin : </strong></td>
                <td width="30%"><?=db_scalar("select count(*) from ngo_code where code_group='ADMIN' and code_cate=1 ");?></td>
                 
                <td width="26%"><?=db_scalar("select count(*) from ngo_code where code_group='ADMIN' and code_cate=13 ");?></td>
              </tr>
              <tr>
                <td><strong>Pin From E-Wallet : </strong></td>
               <td width="30%"><?=db_scalar("select count(*) from ngo_code where code_group='EWALLET' and code_cate=1 ");?></td>
               
               <td width="26%"><?=db_scalar("select count(*) from ngo_code where code_group='EWALLET' and code_cate=13 ");?></td>
              </tr>
              <tr>
                <td><strong>Pin From LR : </strong></td>
                <td width="30%"><?=db_scalar("select count(*) from ngo_code where code_group='LR' and code_cate=1 ");?></td>
                
                <td width="26%"> <?=db_scalar("select count(*) from ngo_code where code_group='LR' and code_cate=13 ");?></td>
              </tr>
              <tr>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
      ...
      <form method="post" name="form1" id="form1" onsubmit="confirm_submit(this)">
        <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="tableList">
          <tr>
			 ...
			 <th width="5%" align="left" nowrap="nowrap">Type</th>
			 <th width="7%" align="left" nowrap="nowrap">Pin slno </th>
			 ...
			 <th width="7%" align="left" nowrap="nowrap">Used ID</th>-->
              <th width="8%" align="left" nowrap="nowrap">LR Tr No  </th>
		      <th width="6%">User ID </th>
		      <th width="17%">User Name </th>
		      <th width="8%">Purcahse By </th>
		      <th width="7%">Issue Date </th> 
		     <th width="7%" align="left" nowrap="nowrap">Exp. Date </th>
 ...
 ?>
          <tr class="<?=$css?>">
                       ...
                       <!--  <td nowrap="nowrap"><?=$code_is?></td>
                        ...
                        <td nowrap="nowrap"><?=$code_creqid?></td>
 						<td align="center"><?=$code_username?></td>
 						<td align="center"><?=$code_username?>
						<?  echo db_scalar("select  u_fname from ngo_users where u_id = '$line[code_userid]'"); ?>						</td>
						<td align="center"><?=$code_purchase_userid?></td>
						<td align="center"><?=$code_date?></td>
 						<td align="center"><?=$code_useto?></td>
					    <td align="center"><?=$code_user?></td>
					    <td align="center"><?=$code_group?></td>
				        <td align="center"><?=$code_is?></td>
