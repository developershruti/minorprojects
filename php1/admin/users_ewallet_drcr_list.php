...
#print_r($_POST);
	...
	if(is_array($arr_pay_ids)) {
		...
		if(isset($_REQUEST['Payment_Paid']) || isset($_REQUEST['Payment_Paid_x']) ){
			$sql_update="update ngo_users_ewallet set pay_status='Paid',pay_transaction_no='$pay_transaction_no',pay_transfer_date=ADDDATE('$pay_transfer_date',INTERVAL 1 MINUTE),pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
 		
		}else if(isset($_REQUEST['Payment_Unpaid']) || isset($_REQUEST['Payment_Unpaid_x']) ){
			$sql_update="update ngo_users_ewallet set pay_status='Unpaid',pay_transaction_no='',pay_transfer_date='',pay_admin='$_SESSION[sess_admin_login_id]' where pay_id in ($str_pay_ids)";
			db_query($sql_update);
		
		}else if(isset($_REQUEST['Payout_SMS']) || isset($_REQUEST['Payout_SMS_x']) ) {
			// sms code start
			 	#$sql_test = "select * from  ngo_users_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
				/*$sql_test = "select u_username,u_fname,u_mobile, sum(pay_amount)as payment  from  ngo_users_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids)  group by pay_userid ";
  			 ...
  			 // sms code start
				...
				$sql_test = "select u_username,u_fname,u_mobile,  pay_amount  as payment  from  ngo_users_ewallet, ngo_users  where pay_userid=u_id and u_mobile!='' and pay_id in ($str_pay_ids) ";
...
$columns = "select u_username,u_fname,u_city,u_id,u_ref_userid,u_bank_register,ngo_users_ewallet.*  ";
$sql = " from ngo_users,ngo_users_ewallet where  u_id=pay_userid ";
//and u_status='Active' and pay_group='$pay_group'
...
<? include("top.inc.php");?>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td id="pageHead"><div id="txtPageHead">   Capital Wallet details </div></td>
        </tr>
      </table>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td id="content"> 
            <form method="get" name="form2" id="form2" onsubmit="return confirm_submit(this)">
              <table width="750"  border="0" align="center" cellpadding="2" cellspacing="0" class="tableSearch">
				...
				<tr>
                  ...
                  <td width="96" align="right"> Payment Group: </td>
                  <td width="161"><? echo  array_dropdown($ARR_WALLET_GROUP,$pay_group,'pay_group',$extra);  ?></td>
				</tr>
                <tr>
                  ...
                  <td width="128" value=""><select name="pay_drcr"   >
                    <option value="">Please Select</option>
                    <option <? if ($pay_drcr=='Cr') { echo "selected";}?> value="Cr">Credit </option>
                    <option  <? if ($pay_drcr=='Dr') { echo "selected";}?>value="Dr">Debit </option>
                  </select></td>
                  <td align="right">Plan: </td>
                  <td><?
			echo array_dropdown( $ARR_WALLET_TYPE, $pay_plan,'pay_plan', 'class="txtbox"  style="width:120px;"','--select--');
			?></td>
                </tr>
			...
			<div align="right"> <a href="users_ewallet_drcr_f.php">Transfer Fund in Fund Wallet</a></div> 
            ...
            <form method="post" name="form1" id="form1" onsubmit="confirm_submit(this)">
              <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="tableList">
                <tr>
                  ...
                  <th width="3%" >Rate</th>
                  <th width="7%" >Amount <?= sort_arrows('pay_amount')?></th>
...
?>
             ...
             <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                <!-- <td width="34%" align="right" style="padding:2px"><h2> Total Capital Wallet Fund ($): <?=$pay_amount_total;?> </h2>
			       
			      </td>
            <td width="34%" align="right" style="padding:2px"> <h2>Total Fund (INR): <?=$pay_amount_total*75;?> </h2>
