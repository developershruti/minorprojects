 ...
 //print_r($_POST);
	...
	if(is_array($arr_gift_ids)) {
		...
		if(isset($_REQUEST['Delete']) || isset($_REQUEST['Delete_x']) ) {
				...
				#db_query($sql_update);
	  //gift_status='New' and
 			 $sql = "delete from ngo_users_gift  where gift_id in ($str_gift_ids)";
			# db_query($sql);
 		} elseif (isset($_REQUEST['Update']) || isset($_REQUEST['Update_x']) ){
		 	if ($submit_status!='') {
			  $sql_update="update ngo_users_gift set gift_status='$submit_status'  where  gift_status='Reject'  and gift_id in ($str_gift_ids)";
 			db_query($sql_update);
			}
			...
			while($line_gen = mysqli_fetch_array($result_gen)) {
				...
				$line2= mysqli_fetch_array($result2);
  				...
  				if ($total_count<5 ) {
					...
					$to_username = db_scalar("select u_username from ngo_users  where  u_id='$gift_userid' ")+0 ;
					$mobile = '91'.$line_gen[u_mobile] ;
					$message = "Dear user ($line_gen[u_username]), you have to send a gift for cycle # $gift_cycle1 to ID No. ($to_username) within 72 hrs. " .SITE_NAME ." team";
...
if ($gift_userid!='') 		{$sql .= " and gift_userid='$gift_userid' ";} 
if ($gift_cycle!='') 	{$sql .= " and gift_cycle='$gift_cycle' "; }
if ($gift_id!='') 		{$sql .= " and gift_id='$gift_id' ";} 
if ($gift_topupid!='') 		{$sql .= " and gift_topupid='$gift_topupid' ";} 
            ...
            <form method="get" name="form2" id="form2" onsubmit="return confirm_submit(this)">
              <table width="676"  border="0" align="center" cellpadding="2" cellspacing="0" class="tableSearch">
                ...
                <tr>
                  ...
                  <td width="137" align="right">Support Topup ID : </td>
                  <td width="244">  <input name="gift_topupid"style="width:120px;" type="text" value="<?=$gift_topupid?>" /> 
				  <?
            ...
            <form method="post" name="form1" id="form1" onsubmit="confirm_submit(this)">
              <table width="100%"  border="0" cellpadding="0" cellspacing="1" class="tableList">
                <tr><th width="4%" >Sl No </th>
                  <th width="5%" >Withdrawal No</th>
                  <th width="9%" >Taker User ID <?= sort_arrows('u_username')?></th>
                    <th width="8%" > Taker Name                      <?= sort_arrows('u_fname')?></th>
                   <th width="5%" >Taker City</th>
 				   <th width="9%" >Giver User ID </th>
				  <th width="9%" >Giver  name</th>
				  <th width="9%" >Giver Mobile</th>
				  <th width="7%" >Provide No  </th>
				 <!-- <th width="7%" >Tr No  </th>
                   <th width="7%" >Tr Date  </th>-->
                   <th width="11%" >Receipt</th>
                   <th width="6%" >Amount </th>
                   <th width="8%" >Status</th>
                   <th width="8%" >Send Date </th>
...
while ($line_raw = mysqli_fetch_array($result)) {
  ...
  ?>
		...
		<td nowrap="nowrap"><?=$gift_refno?></td>
		...
		<td nowrap="nowrap"><?=$line_by[u_mobile]?></td>
		<td nowrap="nowrap"><?=$gift_topupid?></td>
		<!--<td nowrap="nowrap"><?=$gift_tr_no?></td>
		<td nowrap="nowrap"><?=$gift_tr_date?></td>-->
		<td><?  echo $gift_desc; ?> 
		 <? if ($gift_tr_receipt!='') { ?><a href="<?=UP_FILES_WS_PATH.'/profile/'.$gift_tr_receipt?>" target="_blank"><img src="<?=UP_FILES_WS_PATH.'/profile/'.$gift_tr_receipt?>" width="45" /> </a> <? } ?>
		</td>
		<td nowrap="nowrap"><?=$gift_amount?></td>
		<td nowrap="nowrap"><?=$gift_status?></td>
		<td nowrap="nowrap"><?=datetime_format($gift_datetime)?></td>
		<td nowrap="nowrap"><?=date_format2($gift_tr_date)?></td>
                ...
                <tr>
				   ...
				   <td   align="right" valign="top" style="padding:2px"><?
						...
						#echo make_dropdown($sql, 'gift_cycle1', $gift_cycle1,  'class="txtfleid" style="width:150px;" alt="select" emsg="Please Select Pin "', 'Please select');
						  ...
						  <td width="31%" align="center" valign="middle" nowrap="nowrap"  >
						   Update Status : <?=array_dropdown($ARR_GIFT_STATUS, $submit_status, 'submit_status');?>
						  
				  <input name="Update" type="image" id="Update" src="images/buttons/submit.gif" onclick="return updateConfirmFromUser('arr_gift_ids[]')"/>  </td> 
