...
$coin_open = db_scalar("SELECT  round(sett_value,0) FROM ngo_setting where sett_code='COIN_SELL_OPEN' ");
if($coin_open==0) { $arr_error_msgs[] =   "You can not Sell coin at this time, please try it later"; }

#$arr_error_msgs[] =   "You can start sell coin shortly";

$coin_rate = db_scalar("SELECT  sett_value FROM ngo_setting where sett_code='COIN_SELL' ");
//$pay_group_coin='CW'; ///Holding Coin
if($pay_group_coin=='HC') { $arr_error_msgs[] = "Holding Cardano Coin is lock for 1 year from activation date!";}
$pay_group='CW';
...
if ($u_bank_lock=='yes') { 
 ...
 if ($topup_active_count<1) { $arr_error_msgs[] = "Please Activate Your Account With Registration Promo Code"; } 
    ...
    $MINIMUM_INVESTMENT  = 1;
 	if ($total_investment<$MINIMUM_INVESTMENT) { $arr_error_msgs[] = "<br>Minimum coin sell is ".  ($MINIMUM_INVESTMENT); }
 	// and    pay_group='$pay_group'
	# $account_balance = db_scalar("SELECT SUM(IF(pay_drcr='Cr',pay_amount,''))-SUM(IF(pay_drcr='Dr',pay_amount,''))  as Balance FROM ngo_users_ewallet where pay_userid='$_SESSION[sess_uid]'  and pay_group='$pay_group'"); 
  	 if ($total_investment<=0) { $arr_error_msgs[] = "Sell Coin is missing!";}
 	...
 	//  and pay_group='$pay_group_coin'
	 $coin_balance = db_scalar("SELECT SUM(IF(pay_drcr='Cr',pay_amount,''))-SUM(IF(pay_drcr='Dr',pay_amount,''))  as Balance FROM ngo_users_coin where pay_userid='$_SESSION[sess_uid]' and pay_group='$pay_group_coin'"); 
	 if ($total_investment >$coin_balance) {$arr_error_msgs[] =   "Insufficient Coin Balance!";}
...
if (count($arr_error_msgs) ==0) { 
	...
	if ($_POST[Submit]=='Continue') {
		//$arr_error_msgs[] = "Are you sure want to investment - $total_investment";
 		$arr_error_msgs[] = "Are you sure want to sell ".$ARR_COIN_GROUP[$pay_group_coin];
 
		$action = 'Continue';
   
   	} else if  ($_POST[Submit]=='Confirm Payment') {
		$action = 'Confirm';
		//$total_coin 	=round($total_investment/$coin_rate);
		$total_coin 	= $total_investment ;
  
		
		$pay_for1 = "Sell $ARR_COIN_GROUP[$pay_group_coin] @ $coin_rate" ;
		$sql1 = "insert into ngo_users_coin set  pay_drcr='Dr',  pay_userid = '$_SESSION[sess_uid]',pay_refid = '0' ,pay_plan='COIN_SELL' ,pay_group='$pay_group_coin' ,pay_for = '$pay_for1' ,pay_ref_amt='$total_investment' ,pay_unit = 'C', pay_rate = '$coin_rate', pay_amount = '$total_coin',pay_status='Paid' ,pay_date=ADDDATE(now(),INTERVAL 570 MINUTE) ,pay_datetime =ADDDATE(now(),INTERVAL 570 MINUTE) ";
		///exit;
		db_query($sql1);
		$pay_id1 = mysqli_insert_id($GLOBALS['dbcon']);
		///$last_id = mysqli_insert_id($conn);
 		$pay_amount = $total_investment*$coin_rate;
		$arr_error_msgs[] =  "Coin Sell successfully of amount ".price_format($pay_amount)." " ;  
 		$pay_for1 = "Sell $ARR_COIN_GROUP[$pay_group_coin] @$coin_rate | $total_coin coin";
		
		$sql1 = "insert into ngo_users_payment set  pay_drcr='Cr',  pay_userid = '$_SESSION[sess_uid]',pay_refid = '$pay_id1' ,pay_plan='SELL_CG_REVENUE' ,pay_group='SELL_CG_REVENUE' ,pay_for = '$pay_for1' ,pay_ref_amt='$total_investment' ,pay_unit = 'Fix' ,pay_rate = '100', pay_amount = '$pay_amount',pay_status='Paid' ,pay_date=ADDDATE(now(),INTERVAL 570 MINUTE) ,pay_datetime =ADDDATE(now(),INTERVAL 570 MINUTE) ";
		///exit;
		db_query($sql1);
 		$action = '';
		$_POST='';
   ...
   <? include("includes/loader.php")?>
    ...
    <div class="page-top-info">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1>Sell <?=$ARR_COIN_GROUP[$pay_group_coin];?>   </h1>
                    <ol class="breadcrumb">
              			<li class="breadcrumb-item"><a href="index.php">Home</a></li>
              			<li class="breadcrumb-item active" aria-current="page">Coin Sell </li>
            		</ol>
                </div>
            </div>
        </div>
       ...
       <table width="100%" border="0"  cellpadding="0" cellspacing="0"  >
        <tr>
         <td height="200" align="center" valign="" ><form method="post" name="form1" id="form1"  class="contact-form" <?= validate_form()?>  >
		   ...
		   <thead>
					  <tr class="tdhead">
                        <th colspan="2"><strong>Sell <?=$ARR_COIN_GROUP[$pay_group_coin];?> </strong></th>
                      </tr>
                      </thead>
            <?   if ($action == '')  { ?>
            <tr>
             <td style="text-align:right; "   valign="top" class="maintxt" nowrap="nowrap"><strong> Coin Balance : </strong></td>
             <td valign="top" style="text-align:left; " ><span class="blue_link"><strong>
              <?= ($coin_balance)?>
              </strong> </span></td>
            </tr>
            <tr>
             <td style="text-align:right; "  valign="top" class="maintxt" nowrap="nowrap"><strong> Coin Sell Rate: </strong></td>
             <td valign="top" style="text-align:left; "><span class="blue_link"><strong>
              <?=price_format($coin_rate)?>
              </strong> </span></td>
            </tr>
            <tr>
             <td style="text-align:right; "  valign="top" class="maintxt"><strong> Sell QTY: </strong></td>
             <td valign="top" style="text-align:left; ">
			   <input name="total_investment"  type="text" class="txtbox" value="<?=$total_investment?>"   />  
            ...
            <tr>
			 ...
			 <input type="hidden" name="pay_group_coin" value="<?=$pay_group_coin?>" />
              <input name="Submit" type="submit" class="site-btn transition-ease hero" style="color:#000000" value="Continue" /></td>
            </tr>
            <? } else if ($action == 'Continue') { ?>
            <tr>
             <td style="text-align:right; "  valign="top" class="maintxt"><strong>Coin Balance : </strong></td>
           ...
           <br>
           <br>
           <div class="td_box" align="center"> <a href="my_coin_sell.php?pay_group_coin=<?=$pay_group_coin?>">Click Here </a> To Sell again new coin. </div>
           <? }   ?>
          </form>
