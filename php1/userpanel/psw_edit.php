...
if($chkpass==''){
	...
	$msgs="Please enter correct old password";

} else {
    
	if($cnfpassword != $newpassword){
		
		$msgs="Your confirm password does not match with new password";
		
		}else{  
			
		$query="update ngo_users set u_password = '$newpassword' where u_password = '$oldpassword' and u_id= '$_SESSION[sess_uid]'";       $res= db_query($query); 
